...
/**
 * Yeni bir model oluşturan CLI komutu.
 */
class MakeModelCommand extends AbstractMakeCommand
{
    protected static $defaultName = 'make:model';
    protected static $defaultDescription = 'Creates a new model within a specified plugin.';
    protected string $componentType = 'Models';
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $this->writeOutput($output, sprintf('Attempting to create %s: %s in %s', $componentType, $modelName, $pluginSlug === 'database' ? 'main database directory' : 'plugin: ' . $pluginSlug), 'info');
        ...
        $componentFileName = $this->getComponentClassName($modelName) . '.php';
        $componentFilePath = $componentDirPath . '/' . $componentFileName;

        if (file_exists($componentFilePath)) {
            $this->writeOutput($output, sprintf('Warning: %s "%s" already exists at "%s". Aborting.', $componentType, $modelName, $componentFilePath), 'warning');
            return Command::FAILURE;
        }

        $stubPath = BASE_PATH . '/app/Console/Commands/Stubs/Plugin/model.php.stub';
        if (!file_exists($stubPath)) {
            $this->writeOutput($output, sprintf('Error: Model stub file not found: %s', $stubPath), 'error');
            return Command::FAILURE;
        }
        $stubContent = file_get_contents($stubPath);

        $finalContent = $this->prepareComponentContent($stubContent, $pluginSlug, $modelName);
        ...
        $this->writeOutput($output, sprintf('%s "%s" created successfully at "%s".', $componentType, $modelName, $componentFilePath), 'success');
